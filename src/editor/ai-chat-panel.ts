 ...
 * UI only — sends placeholder responses. LLM integration comes later.
 */
...
function addAssistantMessage(text: string, changeCount?: number): void {
  const msg = document.createElement('div');
  ...
  if (changeCount !== undefined && changeCount > 0) {
    ...
    actions.innerHTML = `
      <span class="nk-ai-badge">Applied ${changeCount} change${changeCount !== 1 ? 's' : ''}</span>
      <button class="nk-ai-rollback">↩ Rollback</button>
    `;
    actions.querySelector('.nk-ai-rollback')!.addEventListener('click', () => {
      console.log('[ai-chat] Rollback requested');
    });
...
function sendMessage(): void {
  ...
  setTimeout(() => {
    typing.remove();
    addAssistantMessage('AI integration coming soon — this is a UI preview.', 0);
  }, 500);
}
